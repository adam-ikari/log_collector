...
| 操作 | 文件               | 说明                                                   |
| ---- | ------------------ | ------------------------------------------------------ |
| 修改 | `src/shm_buffer.h` | 替换空壳，写入 shm_init/shm_connect/shm_produce 等声明 |
| 修改 | `src/shm_buffer.c` | 替换空壳，实现共享内存创建、生产者写入、消费者读取     |
| 依赖 | `include/common.h` | 第 1 篇已完成，包含 shm_header_t/log_slot_t 结构体     |
| 依赖 | `src/master.c`     | 第 3 篇已完成，调用 shm_produce 写入共享内存           |

## shm_buffer.h

替换第 1 篇创建的空壳 `src/shm_buffer.h`，写入完整内容：
...
替换第 1 篇创建的空壳 `src/shm_buffer.c`。首先写入 include 和 `shm_init` 函数：
...
int shm_init(shm_header_t **header_out, void **slots_out,
    ...
    if (ftruncate(fd, (off_t)total) < 0) {
        close(fd); shm_unlink(SHM_NAME); return -1;
    }
...
在 `src/shm_buffer.c` 中 `shm_init` 之后添加：

```c
static inline log_slot_t *get_slot(void *slots, uint64_t slot_size,
                                    uint64_t index) {
    return (log_slot_t *)((char *)slots + slot_size * index);
}
```

环形缓冲区中每个槽位大小相同（`slot_size`），第 N 个槽位在 `slots + N * slot_size` 处。`inline` 提示编译器把函数体展开到调用处，避免函数调用开销。

## 生产者写入

继续在 `src/shm_buffer.c` 中添加 `shm_produce`：
...
继续在 `src/shm_buffer.c` 中添加 `shm_consume`：
...
继续在 `src/shm_buffer.c` 中添加 `shm_send_sentinels`：
...
继续在 `src/shm_buffer.c` 中添加 `shm_connect`、`shm_destroy`、`shm_disconnect`：
...
继续在 `src/shm_buffer.c` 中添加 `shm_destroy` 和 `shm_disconnect`：

```c
void shm_destroy(shm_header_t *header, void *slots, uint64_t slot_count) {
    (void)slots; (void)slot_count;
    /* 先销毁信号量，再销毁互斥锁——防止等待信号量的进程访问已销毁的锁 */
...
`shm_destroy` 由 Master 在退出时调用——销毁互斥锁、信号量，unmap 共享内存，unlink 让 `/dev/shm` 中的对象被内核回收。`shm_disconnect` 由 Worker 调用——只 unmap，不做清理工作（锁和信号量由 Master 销毁）。

## 怎么验证

> **注意**：完整的数据收发验证需要等第 5 篇（进程池+文件写入）完成后才能进行。目前可以验证共享内存是否正确创建。
...
$ xxd /dev/shm/log_collector_shm | head -5
00000000: 474f 434c 0100 0000 9800 4000 0000 0000  GOCL......@.....
00000010: 0010 0000 0000 0000 0004 0000 0000 0000  ................
00000020: 0000 0000 0000 0000 0000 0000 0000 0000  ................
00000030: 0000 0000 0000 0000 0000 0000 0000 0000  ................
00000040: 8000 0000 0000 0000 0000 0000 0000 0000  ................
# 前 4 字节：47 4F 43 4C = "GOCL"（"LCOG" 的小端表示，即 0x4C434F47）
# 第 5-8 字节：01 00 00 00 = version 1（小端）
# 第 9-16 字节：98 00 40 00 = 0x00400098 ≈ 4MB buffer_size（小端）
```

> **注意**：如果你的系统没有 `xxd`，可以用 `od -A x -t x1z /dev/shm/log_collector_shm | head -5` 替代。`hexdump` 不是所有系统都有。
