...
**epoll 的 O(1) 来自就绪链表**：内核维护就绪列表，`epoll_wait` 直接取，不遍历所有 fd。边缘触发要求循环读直到 EAGAIN。

**TCP 是字节流，UDP 是数据报**：TCP 需要行缓冲自己拆消息，UDP 天然有边界。这是两种协议的根本区别，不是 epoll 的特性。

**EPOLLIN 必须在 EPOLLRDHUP 之前处理**：先读数据，再关连接。顺序错了数据就丢了。

**`accept`、`O_NONBLOCK`、`EPOLLRDHUP`** 都是 Linux 系统编程的基础。本项目专注于 Linux 平台，充分利用这些 API。

下一篇把共享内存 + 互斥锁 + 信号量组合成高效的环形缓冲区，Master 收到的数据才能交给 Worker 处理。
